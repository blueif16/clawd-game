...
const CLASSIFICATION_SCHEMA = {
  ...
  required: ['prompt', 'archetype', 'coreLoop', 'coreVerb', 'physicsProfile', 'reasoning', 'confidence', 'scoringModel', 'scopeCut'],
  properties: {
    ...
    scoringModel: { type: 'string', enum: ['none', 'bounded-collectible', 'bounded-threshold', 'performance'], description: 'Scoring signal from the prompt GOAL-TYPE (SKILL §2.5) -> W1. Completion defaults to none/bounded-collectible (never a reflexive open counter); performance/survival = performance; explicit target = bounded-threshold.' },
    scopeCut: { type: 'array', minItems: 1, items: { type: 'string', minLength: 1 }, description: 'What is deliberately OUT of v1 (4-8 entries). The anti-slop guardrail. Cuts systems AND multiple levels by default (round one is one rich level); never the juice or the contested decision; does not pre-cut an explicitly-requested level sequence.' },
    mustPreserve: { type: 'string', description: 'Scope-PRESERVE (SKILL §3): the ONE contested decision the cut must not remove, so W1 keeps round one a real game, not a tutorial. A relation, not a per-prompt constant. Omit only for a pure sandbox.' },
  ...
  properties: {
    meta: {
      ...
      properties: {
        ...
        scoringModel: { type: 'string', enum: ['none', 'bounded-collectible', 'bounded-threshold', 'performance'], description: 'Carried from classification (SKILL §3.6). none ⇒ NO score (win is the readout); non-none ⇒ declare maxScore + idempotent rewards.' },
        maxScore: { type: 'number', minimum: 0, description: 'REQUIRED when scoringModel != none: finite reachable Σ of idempotent reward values; score is monotone 0..maxScore. Omit when scoringModel == none.' },
...
Encode these design rules (SKILL §3.5/§3.6/§3.7/§4.5/§4.6): (a) SCORE-MEANING — carry classification.scoringModel into meta.scoringModel; if != none, declare meta.maxScore = Σ reward values (finite, reachable), make every reward idempotent (one-shot + respawn-safe) in its mechanic description, prefer an X/maxScore readout, tie the score to the win, and add the two score assertions (a re-overlap incl. after a respawn leaves score unchanged; score atMost maxScore); if none, declare NO score (the win is the readout). (b) RICHNESS + DIFFICULTY FLOOR — round one is a SUBSTANTIAL single-level game, NOT a thin literal crossing. THE FAILURE TO AVOID (this keeps happening): you place exactly the entities the prompt literally named (e.g. its 3 collectibles + 1 enemy) in a row on ONE screen and stop — a ~30-second first-try win. That is THIN and FAILS. The prompt names the THEME + core loop; ELABORATE a substantial level around it (more instances / length / escalation of the SAME mechanics — never a new SYSTEM the scope-cut forbids). ENUMERATE THE TARGET and build a level with ALL of: a LONG critical path of DISTINCT escalating beats (well beyond one screen); the core threat contesting the path REPEATEDLY (more instances / faster / re-placed, ON the route — never parked in a corner); MULTIPLE genuine risk-vs-reward decisions (carrying classification.mustPreserve); difficulty that RISES across the length (later beats measurably harder — wider gaps / tighter timing / more & faster threats, via SKILL §3.5's per-archetype elaboration levers); an EARNED climax; the M1 teach as only the OPENING FRACTION. The coverage floor is a countable RELATION, never a genre constant: >=3 DISTINCT escalating challenge beats on the critical path (the teach + >=2 more, harder); difficulty rises (later beats measurably harder than the teach); the level is well beyond a single-screen crossing. Then RUN THE §3.7 SELF-CRITIQUE: after drafting, audit the draft against this floor (>=3 distinct beats? threat contests the path repeatedly? multiple contested decisions? difficulty rises? earned climax + teach is only the opening fraction? same loop, no forbidden new system?) — mark each PASS/FAIL with one line of evidence from your own draft, and if ANY is FAIL, ELABORATE (add beats, re-place/multiply the threat onto the later path, escalate the back half, extend PLAN.md's coordinates + the milestones) and re-audit BEFORE returning; only return a draft where all PASS. (c) ONE RICH LEVEL is the DEFAULT — set LEVEL_ORDER:['Level1Scene'] and decompose THAT one level into the 3-5 milestones (escalation is WITHIN the level, not across stages); a multi-level ladder is OPTIONAL and ON-DEMAND — declare extra LEVEL_ORDER entries ONLY if the prompt explicitly asked for multiple levels (then each later level escalates via config + placement + ≤1 small capability, NOT new code, within the ≤5 milestone ceiling). Do NOT default to a ladder, and NEVER manufacture richness by adding stages — richness lives in the depth/length/escalation of the ONE level. (d) BOUNDS (viewport vs world) — the VIEWPORT is the template gameConfig.screenSize (currently 1280×720, full-bleed via Scale.FIT — never changed). The WORLD (layout.bounds) is the viewport for a single-screen level, OR a MULTIPLE of the viewport width for a LONGER scrolling level (e.g. bounds.width = N×1280 for an N-screen side-scroller) with the camera FOLLOWING the player so the viewport stays full at every scroll position (no empty world, no letterbox); world height normally stays one screen (720). A long §3.5 path (well beyond one screen) IMPLIES a wide world: a 3-screen gauntlet ⇒ bounds.width ≈ 3840. State it as the RELATION (viewport = the template screenSize; world = the viewport for one screen, or a multiple of the viewport width for a longer level, camera-follow), never a free 960×540/800×600 and never a cap at one screen.
...
1. JUDGE the design on the numbers — is there a REAL interesting decision (a risk weighed against a reward) AND is the SINGLE level SUBSTANTIAL: PROVE the SUBSTANTIAL-LEVEL FLOOR on the numbers — the reference solution must PASS THROUGH >=3 DISTINCT contested decisions, on a path well BEYOND a single screen, with the LATER beats MEASURABLY HARDER than the teach (escalating difficulty the numbers show — a wider gap / a tighter passable window / a faster/denser threat), ending in an EARNED climax. A winnable path that engages no threat, or only ONE trivial threat and ends in a thin/short ~30-second first-try crossing (the cw1/ceval2 'few named items in a row, one patrol' shape), ⇒ FAIL "too simple"; HARDEN by ADDING/escalating challenge on the path (more contested decisions of the SAME loop, rising difficulty along a longer route; re-derive the reference solution through them), never by weakening, never by adding a scope-cut SYSTEM, never by demanding extra levels — a single rich level is the correct shape. Then: is the threat ON the reward/critical path (NO "undesirable solution": no threat-free path to any reward/goal — statically decidable on the coordinates); is it WINNABLE + FAIR by the per-archetype KINEMATIC FEASIBILITY MATH (every gap/reach/window physically clearable by the documented controls; no soft-lock); is the blueprint COMPLETE. When meta.scoringModel != none, PROVE the score is bounded (maxScore finite, reachable, == Σ reward values; score ≤ maxScore), idempotent (no respawn→re-credit edge; a second overlap leaves score unchanged), and coherent (a score-gated win has gate-threshold ≤ maxScore with the threshold set reachable); when == none, assert no vestigial counter is surfaced. The engine-reuse check is CONDITIONAL: only IF a multi-level ladder is explicitly declared (LEVEL_ORDER > 1 entry) prove engine-reuse (each level a config/placement diff over the shared engine, independently completable, advancing on win to a final game-complete end-state — no inter-level soft-lock); a single rich level (LEVEL_ORDER:['Level1Scene']) is the DEFAULT and VALID — record criterion-9 as a skip ("within-level escalation"), NEVER fail it for "leaving LevelManager unused". Author layout.bounds as the WORLD (distinct from the VIEWPORT): the VIEWPORT is the template screenSize (currently 1280×720, full-bleed via Scale.FIT); the WORLD (layout.bounds) is the viewport for a single-screen level OR an integer MULTIPLE of the viewport width for a LONGER scrolling level (e.g. bounds.width = N×1280) with the camera FOLLOWING the player so the viewport stays full at every scroll position (world height normally 720). A rich/long §2 path well beyond one screen IMPLIES a wide world (a 3-screen gauntlet ⇒ bounds.width ≈ 3840) — HARDEN bounds.width to the multiple the path needs rather than capping it at one viewport; every placement sits inside these bounds.
...
1. Copy templates/core/ into ${PROJECT} first, then OVERLAY templates/modules/<blueprint.meta.archetype> so the module WINS on conflict (per SKILL §2; no-clobber applies only to artifacts already written this run). Fallback to platformer if the module is missing; note it in ${PROJECT}/MEMORY.md. Set package.json.name from meta.title.
