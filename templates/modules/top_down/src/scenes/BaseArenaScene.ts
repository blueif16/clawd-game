...
export abstract class BaseArenaScene extends BaseGameScene {
  ...
  createBaseElements(): void {
    ...
    // NET-NEW (game-omni): status becomes 'playing' once ready latches; the
    // win/lose seam flips it to 'won'/'lost' (template-contract §3.3).
   ...
   * Writes the registry (the single source `window.__GAME__.score` reads) and
   * emits 'scoreChanged' for UIScene integration.
   */
