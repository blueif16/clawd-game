...
export abstract class BaseGameScene extends Phaser.Scene {
   ...
   * NET-NEW (game-omni): the donor had no ready gate — see markReady().
   */
  private _readyLatched: boolean = false;
  ...
  /**
   * Arrow keys (cursors). Both arrows AND WASD drive 8-way movement so W5 can
   * fire 'ArrowLeft'/'ArrowRight'/'ArrowUp'/'ArrowDown' and a human can use WASD.
   */
  ...
  /**
   * Setup input controls (WASD + arrow keys, Space, Shift, E, Q, mouse).
   * Scene-OWNS input; entities read this state, never attach their own
   * listeners. Arrow keys AND WASD both drive movement so W5's arrow inputs
   * and a human's WASD both work.
   */
  protected setupInputs(): void {
    // Disable browser context menu (required for right-click game actions)
    this.input.mouse?.disableContextMenu();

    // Arrow keys (cursors) — read alongside WASD by the FSM.
   ...
   */
  baseUpdate(): void {
    // NET-NEW (game-omni): latch ready on the first interactive frame BEFORE
    // the player-active guard, so window.__GAME__.ready flips true even on the
    // frame the player is being (re)created. See markReady().
    this.markReady();
    ...
    // Update player with input keys (WASD + arrows)
    this.player.update(
      this.wasdKeys,
      this.spaceKey,
      this.shiftKey,
   ...
   *
   * NET-NEW (game-omni): the donor module had no ready gate; W5 boots the
   * built game headless and waits for __GAME__.ready before asserting.
   */
   ...
   * Sets registry `status` = 'lost' (the normalized win/lose seam the hook
   * reads — template-contract §3.3) then shows the game-over screen.
   */
   ...
   * Sets registry `status` = 'won' (the normalized win/lose seam the hook
   * reads — template-contract §3.3) then shows the victory / game-complete
   * screen.
   */
