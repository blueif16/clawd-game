...
| `event` | trigger the REAL interaction the event names (`target:'overlap:player,coin'` → drive held input so the player actually overlaps the coin); use a `commands`-sanctioned trigger ONLY if no natural input reaches it | **NEVER** `setState` the observed outcome directly. The event must happen for real. |
