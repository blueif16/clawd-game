...
- **The RNG seed (`rng.seed`):** `picked = min + (i % (max - min + 1))`; applied via `commands.seed(picked)` (§4). The mechanic under test must be seed-invariant on the observable, so ANY in-band seed is valid.
...
- `permutationsApplied[]` records the ACTUAL moves (omitting point-ranges that couldn't move); `original` is the frozen blueprint value, `permuted` is the in-envelope draw.
