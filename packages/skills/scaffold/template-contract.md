...
- **Preloader loads each slot's on-disk file whenever it has one** — status `generated` (final art) OR `placeholder` (a legible greybox PNG W3 wrote): **a non-empty `path` means "a real file exists — load it"**, regardless of `generated`-vs-`placeholder`. It only **placeholder-fills programmatically** (a colored rect texture under the slot key) the slots with NO file yet (`pending` / no `path`, or a load error), so the project **boots and builds with zero generated art** (W3 may lag / run in parallel). ⚠️ A `placeholder`-status slot WITH a path is a real file — never skip it: skipping it makes every entity render as one type-uniform rect (W3's per-role colors are written to disk but never shown). `status` distinguishes *final vs greybox*; `path` presence distinguishes *file vs no-file* — they are orthogonal.
