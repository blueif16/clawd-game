...
**2026-06-12 — a 3-capture cw1 fix-everything batch (A+B committed, C iterating):** the human played the cloud gnome game `out/cw1` and named severe flaws. **(A) Assets — COMMITTED `819f549`:** real Gemini **batch** generation is now the W3 DEFAULT (placeholder = graceful floor) via a new self-contained tool `packages/skills/assets/gen/` — proven with real cw1 art. **(B) Fullscreen — COMMITTED `f5104d0`:** the game fills the window (16:9 **1280×720**; camera/world bounds derive from `screenSize`). **(C) Game-design foundations — STAGED, iterating:** a new grounding doc `research/game-design-foundations.md` (812 lines, 38 sources) is encoded into the DESIGN nodes (W0/W1/VERIFY-1). A MiniMax-M3 clean-room re-run (`out/ceval1`) **validated the score-meaning fix** (`meta.maxScore` + idempotent rewards — the farming bug is fixed) and bounds=1280×720, but W1 still shipped one short level. **Human pivot:** drop the multi-level ladder — make ROUND ONE harder/longer/content-rich (a complete game in ONE level); multi-level stays a cheap on-demand scaffold extension. Re-editing the difficulty doctrine + re-running W1 before committing C.

**NEXT:**
1. **Finish Capture C (the design doctrine).** Re-edit W0/W1/VERIFY-1 + the criteria fixture to the **single-rich-level** difficulty doctrine — round one is harder/longer/content-rich (escalating challenge + multiple contested decisions in ONE level), multi-level a cheap on-demand scaffold extension; re-run W1 on MiniMax-M3, inspect, then commit C as a dated/sha'd diagnostics line. _(A assets `819f549` + B fullscreen `f5104d0` already committed.)_
2. **Fresh end-to-end run that BUILDS the new design** — prove W3 ships real assets, the game fills the window, and W4 builds a ≥2-level ladder with a bounded/idempotent score (the multi-level + scaffold `LevelManager` wiring the design now declares).
3. Build the other 4 genre templates (top_down / grid_logic / tower_defense / ui_heavy), reusing `templates/core/` unchanged → **`docs/handoff-build-archetype-templates.md`**.
4. Human is the eye: inspect the generated artifact + screenshots for play-feel quality; route any real flaw via `hermes-skill-system` (a wave → its SKILL; the chain → `game-omni.js`).
