...
_Updated 2026-06-10. Single entry point per session. **Read order:** this → `design/pipeline-design-v1.md` (the plan) → `research/` (the why). Keep this LEAN: research lives in `research/`, the plan lives in `design/`._
...
**NEXT:**
1. **Fresh end-to-end validation run on the new 7-node chain** (chromium installed) — on a fresh prompt (never a td1 rerun, to avoid overfitting): prove VERIFY-1 freezes a winnable `blueprint.json`, W4 builds it verbatim, and VERIFY-2's perturbation gate catches a deliberately-contorted build. Also measures the open R3 (baked-config rebuild cost on the Pi budget).
2. Build the other 4 genre templates (top_down / grid_logic / tower_defense / ui_heavy), reusing `templates/core/` unchanged → hand-off prompt **`docs/handoff-build-archetype-templates.md`**. _(td1 exercised `top_down` via the archetype overlay — confirm the dedicated template path next.)_
